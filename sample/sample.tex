...
\collaborationName{Friends of AASTeX}
%% will print "The AAS collaboration" after the shortened author list.
%% Note that all the \altaffil information will still be shown so it
%% has to be manually commented out if you do not want it shown.
%%
...
%% Use \author, \affil, and the \and command to format author and affiliation 
...
%% The necessary command are \AuthorCallLimit and \collaborationName.
%% An \AuthorCallLimit=2 call prior to the author list will only show
%% the authors in the first two \author calls.  The \collaborationName
%% defines the collaboration identifier.  Commented examples below.
...
alignment you need to include the {\tt\string\decimals} command after
...
used \break 
\edit3{to show this changed text}.  Since over 90\% of all manuscripts
